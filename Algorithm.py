# Algoritmo criado para detectar exoplanetas usando o método de trânsito
# Como referência, foram utilizados os dados do site: https://archive.stsci.edu/prepds/tess-data-alerts/index.html#dataaccess
...
columnTic = pd.read_csv("hlsp_tess-data-alerts_tess_phot_alert-summary-s01+s02+s03+s04_tess_v9_spoc.csv")

target = columnTic["#tic_id"].unique() # alvo de observação
listTarget = [target]
parameters = itertools.product(*listTarget)

# Função para plotagem dos resultados
...
def returResults(results):
    ...
    print('The number of transits', results.transit_count, 'with/without intransit data points:', results.distinct_transit_count, '/', results.empty_transit_count)
...
# Função principal TLS - Transit Least Squaresp
def methodTLS(allDatasLighCurve):
    ...
    try:
        ...
        intransit = transit_mask(alltime, results.period, 2*results.duration, results.T0)
        allflux_second_run = allflux[~intransit]
        alltime_second_run = alltime[~intransit]
        alltime_second_run, allflux_second_run = cleaned_array(alltime_second_run, allflux_second_run)
        
        model_second_run = transitleastsquares(alltime_second_run, allflux_second_run)
...
if __name__ == '__main__':
    ...
    for value in parameters:
        ...
        try:
            ...
            allCurve = lk.search_lightcurve("TIC 55652896", author="SPOC", sector = [3,4,5]) # Nesta lista, cada linha representa um período de observação diferente
            pp.pprint(allCurve) 

            allObservationSectors = allCurve.download_all()# baixar os dados de todos os setores
    ...
    print('\nTempo utilizado para processar as curvas de luz: %f seconds\n' %t)
    

# Confirmar os resultados ou no site: https://exofop.ipac.caltech.edu/tess/target.php?id=55652896
